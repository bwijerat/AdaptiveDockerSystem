...
def get_stats(services, sql_cpu_usages, sql_mem_usages, web_worker_cpu_usages, web_worker_mem_usages, sql_cpu_avg,
    ...
    for task in services["web-worker"]["tasks"]:
        with urlopen('http://{node}/containers/{containerID}/stats?stream=false'.format(
                node=node_list[task["NodeID"]], containerID=task["ContainerID"])) as url:
            data = json.loads(url.read().decode())
...
def store_stats(sql_cpu_avg, sql_mem_avg, web_worker_mem_avg, web_worker_cpu_avg, num_web_workers, num_sql,
...
def controller(input_pipe, output_pipe, number_of_processes):
    ...
    # Start generating a load (pick 1 random process to run on 1 random node)
